...
export type InvertedEventType<T extends Record<string, string>> = {
  [P in keyof T as T[P]]: P
}

// 应用反转类型到 Canvas 事件
export type InvertedEventNames<Events extends Record<string, any>> = InvertedEventType<
  RawEventNames<Events>
>

// 创建最终的 Canvas 事件类型，直接使用 CanvasEvents 中的类型定义
export type AllEvents<Events extends Record<string, any>> = {
  [K in keyof InvertedEventNames<Events>]: (
    opt: Events[InvertedEventNames<Events>[K] & keyof Events]
  ) => void
}
